...
export const buyProducts = (data) => new Promise(async (resolve, reject) => {
    try {
        ...
        const response = await Promise.all([
            db.Bill.create({ total: parseInt(data?.total), uid: data.uid, status: 'Pending', id: bid, coupon: data.coupon + '', isChecked: false }),
...
export const getDashboard = ({ days, type, from, to }) => new Promise(async (resolve, reject) => {
    try {
...
export const changeStatus = (data, id) => new Promise(async (resolve, reject) => {
    try {
        const response = await db.Bill.update({ ...data, isChecked: true }, { where: { id } })
        resolve({
            err: response ? 0 : 1,
...
export const getCartAnon = (pids) => new Promise(async (resolve, reject) => {
    try {
        const response = await db.Product.findAll({
            where: { id: pids }
        })
