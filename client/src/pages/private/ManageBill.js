...
const ManageBill = () => {
    ...
    const handleSubmit = async () => {
        const data = { status }
        if (reasonCanceled) {
            data.reasonCancel = reasonCanceled
        }
    ...
    return (
        <div className='w-full h-full relative'>
            ...
            <div className='flex flex-col mt-4 relative'>
                {status === 'Failed' && <div className='absolute top-0 bottom-0 left-0 right-0 bg-white'>
                    <h4>Lý do hủy đơn hàng:</h4>
                    <input
                        type="text"
                        value={reasonCanceled} onChange={e => setReasonCanceled(e.target.value)}
                ...
                <div className='flex items-center justify-center px-2 py-4 border-t'>
                    ...
                    <span className='flex-1 font-medium flex justify-end'>Lý do hủy đơn</span>
                </div>
                {billdata?.map(el => (
                    <div
                    ...
                    >
                        ...
                        <span className='flex-1 flex justify-center'>{el.reasonCancel || ''}</span>
                    </div>
                ))}
