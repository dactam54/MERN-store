...
import { formatVietnameseToString, renderStar } from '../utils/fn'
...
const Card = ({ name, price, image, policy, star, discountNumber, id, wishlist, isLike, voterCount }) => {
    ...
    const updateCart = async () => {
        if (isLoggedIn) {
            ...
            dispatch({ type: actionTypes.CART_ANON, pid: id })
        }
    }
        ...
        >
            ...
            <div className='w-full items-center flex justify-between'>
                {isLoggedIn
                    ...
                    : <>
                        {anonmyousCart && anonmyousCart?.some(i => i === id) ? <span
                            className='flex gap-2 items-center text-main  hover:text-main'
                            onClick={(e) => {
                                e.stopPropagation()
                                updateCart()
